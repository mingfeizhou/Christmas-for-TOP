...
class ParticleItem {
  ...
  slotIndex: number;
  // Specific randoms for photos to place them on the tree surface
  treePlacement?: {
    heightT: number; // 0 to 1
    angleOffset: number;
    depthOffset: number;
  };
  ...
  constructor(mesh: THREE.Mesh, type: 'DECOR' | 'PHOTO' | 'DUST', index: number) {
    ...
    if (type === 'PHOTO') {
      // Place photos randomly on the tree cone
      // Avoid the very top (too narrow)
      this.treePlacement = {
        heightT: Math.random() * 0.85, 
        angleOffset: Math.random() * Math.PI * 2,
        depthOffset: 0.5 + Math.random() * 0.5 // Slightly outside the core decor
      };
    }
  }
    ...
    const calculateTargets = () => {
      ...
      particles.forEach((p, idx) => {
        if (mode === SceneMode.TREE) {
          ...
          } else if (p.type === 'PHOTO' && p.treePlacement) {
            // Place on tree surface
            const t = p.treePlacement.heightT;
            const radius = 13 * (1 - t) + p.treePlacement.depthOffset;
            const angle = t * 50 * Math.PI + p.treePlacement.angleOffset + time * 0.5;
